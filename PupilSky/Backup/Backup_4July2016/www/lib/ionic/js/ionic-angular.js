 ...
 *
 * Put the content of the model inside of an `<ion-model-view>` element.
 *
 * **Notes:**
 * - A model will broadcast 'model.shown', 'model.hidden', and 'model.removed' events from its originating
 * scope, passing in itself as an event argument. Both the model.removed and model.hidden events are
 * called when the model is removed.
 *
 * - This example assumes your model is in your main index file or another template file. If it is in its own
 * template file, remove the script tags and call it by file name.
 *
 * @usage
 * ```html
 * <script id="my-model.html" type="text/ng-template">
 *   <ion-model-view>
 *     <ion-header-bar>
 ...
 *   $ionicModal.fromTemplateUrl('my-model.html', {
 *     scope: $scope,
 ...
 *     $scope.model.hide();
 *   };
 *   // Cleanup the model when we're done with it!
 *   $scope.$on('$destroy', function() {
 *     $scope.model.remove();
 *   });
 *   // Execute action on hide model
 *   $scope.$on('model.hidden', function() {
 *     // Execute action
 *   });
 *   // Execute action on remove model
 *   $scope.$on('model.removed', function() {
 *     // Execute action
 *   });
 * });
 * ```
...
function($rootScope, $ionicBody, $compile, $timeout, $ionicPlatform, $ionicTemplateLoader, $$q, $log, $ionicClickBlock, $window, IONIC_BACK_PRIORITY) {
   ...
   *
   * Be sure to call [remove()](#remove) when you are done with each model
   * to clean it up and avoid memory leaks.
   *
   * Note: a model will broadcast 'model.shown', 'model.hidden', and 'model.removed' events from its originating
   * scope, passing in itself as an event argument. Note: both model.removed and model.hidden are
   * called when the model is removed.
   */
  var ModalView = ionic.views.Modal.inherit({
     ...
     * @description Creates a new model controller instance.
     ...
     *    the model when shown. Will only show the keyboard on iOS, to force the keyboard to show
     *    on Android, please use the [Ionic keyboard plugin](https://github.com/driftyco/ionic-plugin-keyboard#keyboardshow).
     *    Default: false.
     *  - `{boolean=}` `backdropClickToClose` Whether to close the model on clicking the backdrop.
     *    Default: true.
     *  - `{boolean=}` `hardwareBackButtonClose` Whether the model can be closed using the hardware
     ...
     * @description Show this model instance.
     * @returns {promise} A promise which is resolved when the model is finished animating in.
     */
    show: function(target) {
      ...
      // if model was closed while the keyboard was up, reset scroll view on
     ...
     * @description Hide this model instance.
     * @returns {promise} A promise which is resolved when the model is finished animating out.
     */
    hide: function() {
      var self = this;
      var modalEl = jqLite(self.modalEl);
     ...
     * @description Remove this model instance from the DOM and clean up.
     * @returns {promise} A promise which is resolved when the model is finished animating out.
     */
    remove: function() {
      var self = this,
      ...
      // Only hide model, when it is actually shown!
      ...
      // We don't want that, if the model window is already hidden.
      if (self._isShown) {
        promise = self.hide();
      } else {
        deferred = $$q.defer();
     ...
     * @returns boolean Whether this model is currently shown.
     */
    isShown: function() {
  ...
  return {
     ...
     * @param {string} templateString The template string to use as the model's
     * content.
  ...
  return {
    $get: ['$q', '$ionicScrollDelegate', function($q, $ionicScrollDelegate) {
      var self = {
         ...
         * or close a model which may be open.
         *
...
function($scope, $element, $attrs, $compile, $rootScope) {
  ...
  self.init = function() {
    ...
    // don't bother if inside a model or there's no parent navView
...
.directive('ionModal', [function() {
  return {
    ...
    replace: true,
    controller: [function() {}],
    template: '<div class="model-backdrop">' +
                '<div class="model-backdrop-bg"></div>' +
                '<div class="model-wrapper" ng-transclude></div>' +
              '</div>'
  };
}]);
